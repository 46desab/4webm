...
-k KEYFRAME	Specifies the I-frame spacing for the SVT-VP9 encoder.
   		DEFAULT:        (automatic)
		EXAMPLE:	-k 60
...
AudioEncode() {
    ARG2="$2"
    echo "Pass 1/2:"
    ffmpeg -hide_banner -loglevel error -stats -i "$INFILE" $STARG $ETARG -vn $AUDIOPTS "audio_intermediate.opus"
    echo "Pass 2/2:"
    ffmpeg -hide_banner -loglevel error -stats -i "$1" -i "audio_intermediate.opus" -map 0:v:0 -c:v copy $ARG2 -map 1:a:0 -shortest -map_metadata -1 -y "${OUTFILE}_reencode.webm"
    rm "audio_intermediate.opus"
}

SvtVp9Encode() {
    BITRATE=$( echo "$BITRATE * 1000" | bc )
    BSFV="-bsf:v vp9_superframe"
    ...
    if [[ -z $KEYSPACE ]]
    then
	KEYSPACE=$( echo "$FRATE * 8 - 1" | bc )
    else
	KEYSPACE=$( echo "$KEYSPACE - 1 " | bc )
    fi
    ...
    echo -e "\nEncoding, Stage 2/2:"
    $SVTDIR/SvtVp9EncApp -i raw.yuv -w $HRES -h $VRES -intra-period $KEYSPACE -fps-num $NUM -fps-denom $DENOM -rc 1 -tbr $BITRATE -min-qp 0 -max-qp 60 -tune 0 -enc-mode $SPEED -b "intermediate.ivf"

    if [[ $AUDIO == true ]]
    then
	echo -e "\nMuxing audio 1/1:"
	AudioEncode "intermediate.ivf" "$BSFV"
	rm "intermediate.ivf"
	rm raw.yuv
	OutfileSize $( echo "${OUTFILEFIXED}_reencode.webm" | sed 's/^.*\///' )
    else
	ffmpeg -i "intermediate.ivf" -c:v copy $BSFV -map_metadata -1 -y "${OUTFILE}.webm"
	rm "intermediate.ivf"
	rm raw.yuv
	OutfileSize $( echo "${OUTFILEFIXED}.webm" | sed 's/^.*\///' )
    fi
...
while getopts "ab:e:fi:k:lm:o:q:s:tv:x:h" OPTS
do
    case "$OPTS" in
	a) AUDIO=true
	   eval NEXTOPT=${!OPTIND}
...
ResolutionCheck() {
    ...
    if [[ $libcv == "svt-vp9" ]] && [[ $( echo "$HRES%8" | bc ) -ne 0 || $( echo "$VRES%8" | bc ) -ne 0 ]]
    then
	LIBCV="libvpx-vp9"
	OVERHEAD=3
    fi
